...
public class PersonController {
    ...
    //@Autowired
    @Resource    // Resource和@Autowired都是做bean的注入时使用，其实@Resource并不是Spring的注解，它的包是javax.annotation.Resource，需要导入，但是Spring支持该注解的注入。
    private QinNiuConfig qinNiuConfig;//属性封装到对象中提取
    @Autowired
    ...
    public String qiNiu(@PathVariable("id") String fileName){
        System.out.println(qinNiuConfig);
    ...
    /**
     * templates的展示
     *  失败 ，原因 需要用 @Controller
     * * @RestController是@Controller和@ResponseBody的结合体，两个标注合并起来的作用。 @ResponseBody 加载Controller表明整个的Controller中都是返回数据而不是去找相应的页面地址！！！！
     * @return
     */
